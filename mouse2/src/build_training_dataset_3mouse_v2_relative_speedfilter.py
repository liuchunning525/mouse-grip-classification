...
def infer_condition_from_path_or_data(path, data=None):
    ...
    for part in Path(path).parts:
        ...
        if low in ["condition a", "condition_a", "conditiona"]:
            return "A"
        if low in ["condition b", "condition_b", "conditionb"]:
            return "B"
        if low in ["condition c", "condition_c", "conditionc"]:
            return "C"
        if low in ["condition d", "condition_d", "conditiond"]:
            return "D"
    name = Path(path).name
    if "_A_" in name:
        return "A"
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Skip trials whose mouse_peak_speed is greater than this threshold. Example: 180 or 220."
    )
    parser.add_argument(
        "--max_mean_speed",
        type=float,
        default=None,
        help="Skip trials whose mouse_mean_speed is greater than this threshold."
    )
    ...
    for file in files:
        ...
        if label is None:
            ...
            if label is not None:
                label = LABEL_ALIAS.get(str(label), str(label))

        if label not in {"palm", "claw", "fingertip"}:
        ...
        for trial in data.get("trials", []):
            ...
            peak_speed = get(features, "mouse_peak_speed", 0.0)
            mean_speed = get(features, "mouse_mean_speed", 0.0)

            if args.max_peak_speed is not None and peak_speed > args.max_peak_speed:
                skipped += 1
                skipped_by_speed += 1
                continue

            if args.max_mean_speed is not None and mean_speed > args.max_mean_speed:
                skipped += 1
                skipped_by_speed += 1
                continue
    ...
    if args.max_peak_speed is not None or args.max_mean_speed is not None:
        print("Skipped by speed:", skipped_by_speed)
        print("Max peak speed:", args.max_peak_speed)
        print("Max mean speed:", args.max_mean_speed)
